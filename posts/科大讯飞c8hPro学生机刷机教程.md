...
[^3]: 下载好fdl文件所在的压缩包后，解压，直接把1、2两个文件放在flash_tool根目录，不要再套一个文件夹

[^4]: 在Driver_R4.21.3201.zip里面


[^5]: 在刷机时，`.fdl`文件负责在芯片和电脑之间建立连接，并引导后续的刷机过程，所以如果刷机的命令窗口不小心关闭了，重做[[科大讯飞c8hPro学生机刷机教程#2. **推送fdl文件：**[ 5]]]就行，确保之后的命令都是在`FDL2>`模式下进行的。
